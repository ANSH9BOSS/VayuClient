...
extends Screen {
    private static final Identifier LOGO_TEXTURE = Identifier.fromNamespaceAndPath((String)"vayuclient-hud", (String)"textures/gui/title-vayuclient-logo.png");
    private static final Identifier DIAMOND_TEXTURE = Identifier.fromNamespaceAndPath((String)"vayuclient-hud", (String)"textures/gui/title/diamond.png");
    private static final Identifier SEARCH_ICON = Identifier.fromNamespaceAndPath((String)"vayuclient-hud", (String)"textures/gui/title/search.png");
    ...
    public StoreScreen(Screen previousScreen) {
        super((Component)Component.literal((String)"VayuClient Store"));
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        graphics.fill(0, 0, DisplaySpace.width(), DisplaySpace.height(), VayuHUDUI.withAlpha(-16777216, 55));
    }
    ...
    private void drawPanelBackground(GuiGraphicsExtractor graphics, int alpha) {
        graphics.fill(this.panelX, this.panelY, this.panelX + this.panelWidth, this.panelY + this.panelHeight, VayuHUDUI.withAlpha(-234156528, alpha));
        graphics.fill(this.panelX, this.panelY, this.panelX + this.panelWidth, this.panelY + 126, VayuHUDUI.withAlpha(-15986412, Math.min(alpha, 230)));
        graphics.fill(this.panelX, this.gridY() - 1, this.panelX + this.panelWidth, this.gridY(), VayuHUDUI.withAlpha(1143616571, alpha));
    }
    ...
    private void drawHeader(GuiGraphicsExtractor graphics, int alpha, int mouseX, int mouseY) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, DisplaySpace.texture(LOGO_TEXTURE), 0, 0, 0.0f, 0.0f, 510, 161, 510, 161, VayuHUDUI.withAlpha(-1, alpha));
        graphics.pose().popMatrix();
        int storeBtnW = this.panelWidth < 1050 ? 152 : 168;
        graphics.fill(this.panelX, headerY, this.panelX + storeBtnW, headerY + headerH, VayuHUDUI.withAlpha(-16723201, alpha));
        graphics.text(this.font, "Cosmetic Store", this.panelX + 14, headerY + 12, VayuHUDUI.withAlpha(-723724, alpha), true);
        int coinX = this.panelX + storeBtnW + 8;
        graphics.fill(coinX, headerY, coinX + 112, headerY + headerH, VayuHUDUI.withAlpha(-435153640, alpha));
        int iconSize = 18;
        graphics.blit(RenderPipelines.GUI_TEXTURED, DisplaySpace.texture(DIAMOND_TEXTURE), coinX + 11, headerY + 7, 0.0f, 0.0f, iconSize, iconSize, iconSize, iconSize, VayuHUDUI.withAlpha(-13312, alpha));
        String coinText = String.valueOf(this.state.getCoinBalance());
        graphics.text(this.font, coinText, coinX + 36, headerY + 12, VayuHUDUI.withAlpha(-723724, alpha), true);
        ...
        int claimBg = canClaim ? VayuHUDUI.withAlpha(claimHovered ? -13058312 : -16723201, alpha) : VayuHUDUI.withAlpha(claimHovered ? -266722777 : -435153640, alpha);
        ...
        graphics.text(this.font, claimText, claimX + (118 - this.font.width(claimText)) / 2, headerY + 12, VayuHUDUI.withAlpha(canClaim ? -723724 : -9934744, alpha), true);
        ...
        graphics.fill(closeX, closeY, closeX + 30, closeY + 30, VayuHUDUI.withAlpha(closeHovered ? -266722777 : -435153640, alpha));
        int n = closeX + (30 - this.font.width("X")) / 2;
        Objects.requireNonNull(this.font);
        graphics.text(this.font, "X", n, closeY + (30 - 9) / 2 + 1, VayuHUDUI.withAlpha(closeHovered ? -723724 : -7303024, alpha), true);
    }
    ...
    private void drawCategoryTabs(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int alpha, float dt) {
        ...
        for (int i = 0; i < categories.length; ++i) {
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, categoryIcon, tabX, tabY, 0.0f, 0.0f, tabSize, tabSize, tabSize, tabSize, VayuHUDUI.withAlpha(-1, alpha));
            if (isSelected) {
                graphics.fill(tabX + 4, tabY + tabSize - 3, tabX + tabSize - 4, tabY + tabSize - 1, VayuHUDUI.withAlpha(-13058312, alpha));
            } else if (isHovered) {
                VayuHUDUI.outline(graphics, tabX, tabY, tabSize, tabSize, VayuHUDUI.withAlpha(1143616571, alpha));
            }
            tabX += tabSize + tabGap;
        }
    ...
    private void drawSearchBar(GuiGraphicsExtractor graphics, int alpha, int mouseX, int mouseY) {
        ...
        int searchBg = this.searchFocused ? VayuHUDUI.withAlpha(-266722777, alpha) : VayuHUDUI.withAlpha(-435153640, alpha);
        graphics.fill(searchBarX, searchBarY, searchBarX + searchW, searchBarY + searchH, searchBg);
        VayuHUDUI.outline(graphics, searchBarX, searchBarY, searchW, searchH, this.searchFocused ? VayuHUDUI.withAlpha(-16723201, alpha) : VayuHUDUI.withAlpha(1143616571, alpha));
        int iconSize = 14;
        graphics.blit(RenderPipelines.GUI_TEXTURED, DisplaySpace.texture(SEARCH_ICON), searchBarX + 10, searchBarY + 8, 0.0f, 0.0f, iconSize, iconSize, iconSize, iconSize, VayuHUDUI.withAlpha(-1, alpha));
        int textX = searchBarX + 32;
        int textY = searchBarY + 11;
        if (this.searchQuery.isEmpty() && !this.searchFocused) {
            graphics.text(this.font, "Search cosmetics...", textX, textY, VayuHUDUI.withAlpha(-7303024, alpha), false);
        } else {
            long time;
            String displayText = this.fitText(this.searchQuery, searchW - 44);
            graphics.text(this.font, displayText, textX, textY, VayuHUDUI.withAlpha(-723724, alpha), false);
            if (this.searchFocused && ((time = System.currentTimeMillis()) - this.cursorBlinkTime) % 1000L < 500L) {
                ...
                graphics.fill(cursorX, textY - 2, cursorX + 1, textY + 14, VayuHUDUI.withAlpha(-723724, alpha));
            }
        }
        boolean bl = ownedHovered = mouseX >= layout.ownedX && mouseX <= layout.ownedX + 108 && mouseY >= searchBarY && mouseY <= searchBarY + searchH;
        int ownedBg = this.ownedOnly ? VayuHUDUI.withAlpha(-15511009, alpha) : VayuHUDUI.withAlpha(ownedHovered ? -266722777 : -435153640, alpha);
        ...
        graphics.text(this.font, ownedText, layout.ownedX + (108 - this.font.width(ownedText)) / 2, searchBarY + 11, VayuHUDUI.withAlpha(this.ownedOnly ? -723724 : -7303024, alpha), false);
        ...
        int removeBg = VayuHUDUI.withAlpha(hasEquipped && removeHovered ? -16723201 : -435153640, alpha);
        graphics.fill(layout.removeAllX, searchBarY, layout.removeAllX + 104, searchBarY + searchH, removeBg);
        String removeText = "Remove All";
        graphics.text(this.font, removeText, layout.removeAllX + (104 - this.font.width(removeText)) / 2, searchBarY + 11, VayuHUDUI.withAlpha(hasEquipped && removeHovered ? -723724 : -7303024, alpha), false);
    }
    ...
    private void drawStatus(GuiGraphicsExtractor graphics, int alpha) {
        ...
        graphics.text(this.font, message, statusX, statusY, VayuHUDUI.withAlpha(-7303024, alpha), false);
    }
    ...
    private void drawSelectedCategoryLabel(GuiGraphicsExtractor graphics, CosmeticCategory category, int y, int alpha) {
        ...
        graphics.text(this.font, label, x, y, VayuHUDUI.withAlpha(-723724, alpha), true);
        graphics.text(this.font, countText, x + this.font.width(label) + 10, y, VayuHUDUI.withAlpha(-7303024, alpha), false);
    }
    ...
    private Identifier categoryIcon(CosmeticCategory category, boolean hover) {
        ...
        return Identifier.fromNamespaceAndPath((String)"vayuclient-hud", (String)("textures/gui/store/categories/" + category.getIconName() + suffix + ".png"));
    }
    ...
    private void drawEmptyState(GuiGraphicsExtractor graphics, int alpha) {
        ...
        graphics.text(this.font, title, centerX - this.font.width(title) / 2, centerY - 10, VayuHUDUI.withAlpha(-723724, alpha), true);
        graphics.text(this.font, subtitle, centerX - this.font.width(subtitle) / 2, centerY + 6, VayuHUDUI.withAlpha(-7303024, alpha), false);
    }
    ...
    private void renderScrollbar(GuiGraphicsExtractor graphics) {
        ...
        int thumbColor = this.scrollbarDragging ? -16723201 : -1722656160;
    ...
    private void drawCard(GuiGraphicsExtractor graphics, StoreCosmetic cosmetic, int x, int y, int w, int h, int mouseX, int mouseY, int alpha) {
        ...
        int cardBg = VayuHUDUI.withAlpha(hovered ? -434955745 : -653586413, alpha);
        graphics.fill(x, y, x + w, y + h, cardBg);
        VayuHUDUI.outline(graphics, x, y, w, h, VayuHUDUI.withAlpha(hovered ? 1717331565 : 1143616571, alpha));
        if (hovered) {
            graphics.fill(x + 1, y + h - 3, x + w - 1, y + h - 1, VayuHUDUI.withAlpha(-13058312, alpha));
        }
        int rarityColor = VayuHUDUI.withAlpha(cosmetic.getRarity().getColor(), alpha);
        graphics.fill(x + 1, y + 1, x + w - 1, y + 4, rarityColor);
        int previewSize = Math.min(w - 34, h - 92);
        int previewX = x + (w - previewSize) / 2;
        int previewY = y + 15;
        int framePad = 6;
        int frameColor = VayuHUDUI.blend(-435153640, cosmetic.getRarity().getColor(), hovered ? 0.18f : 0.08f);
        graphics.fill(previewX - framePad, previewY - framePad, previewX + previewSize + framePad, previewY + previewSize + framePad, VayuHUDUI.withAlpha(frameColor, alpha));
        VayuHUDUI.outline(graphics, previewX - framePad, previewY - framePad, previewSize + framePad * 2, previewSize + framePad * 2, VayuHUDUI.withAlpha(hovered ? cosmetic.getRarity().getColor() : 1143616571, alpha));
        String assetName = cosmetic.getPreviewAssetName();
        if (assetName != null && !assetName.isEmpty()) {
            Identifier previewId = Identifier.fromNamespaceAndPath((String)"vayuclient-hud", (String)("textures/gui/store/items/" + assetName + ".png"));
            if (this.hasTexture(previewId)) {
                graphics.blit(RenderPipelines.GUI_TEXTURED, previewId, previewX, previewY, 0.0f, 0.0f, previewSize, previewSize, previewSize, previewSize, VayuHUDUI.withAlpha(-1, alpha));
            } else {
        ...
        graphics.text(this.font, displayName, x + (w - this.font.width(displayName)) / 2, nameY, VayuHUDUI.withAlpha(-723724, alpha), true);
        int rarityY = nameY + 14;
        String rarityText = cosmetic.getRarity().getDisplayName();
        graphics.text(this.font, rarityText, x + (w - this.font.width(rarityText)) / 2, rarityY, VayuHUDUI.withAlpha(cosmetic.getRarity().getColor(), alpha), false);
        int btnY = y + h - 30;
        int btnH = 22;
        int btnW = w - 20;
        int btnX = x + 10;
        ...
        if (owned && equipped) {
            ...
            int eqBg = VayuHUDUI.withAlpha(-15511009, alpha);
            ...
            graphics.text(this.font, fittedEqText, btnX + (equippedBtnW - this.font.width(fittedEqText)) / 2, btnY + 5, VayuHUDUI.withAlpha(-723724, alpha), true);
            boolean removeHovered = mouseX >= removeBtnX && mouseX <= removeBtnX + removeBtnW && mouseY >= btnY && mouseY <= btnY + btnH;
            int removeBg = VayuHUDUI.withAlpha(removeHovered ? -16723201 : -435153640, alpha);
            ...
            graphics.text(this.font, fittedRemoveText, removeBtnX + (removeBtnW - this.font.width(fittedRemoveText)) / 2, btnY + 5, VayuHUDUI.withAlpha(removeHovered ? -723724 : -7303024, alpha), false);
        } else if (owned) {
            int eqBg = VayuHUDUI.withAlpha(hovered ? -15243738 : -435153640, alpha);
            graphics.fill(btnX, btnY, btnX + btnW, btnY + btnH, eqBg);
            String eqText = "Equip";
            graphics.text(this.font, eqText, btnX + (btnW - this.font.width(eqText)) / 2, btnY + 5, VayuHUDUI.withAlpha(hovered ? -723724 : -7303024, alpha), true);
        } else {
            boolean canAfford;
            boolean bl = canAfford = this.state.getCoinBalance() >= cosmetic.getPrice();
            int buyBg = canAfford ? VayuHUDUI.withAlpha(hovered ? -13058312 : -16723201, alpha) : VayuHUDUI.withAlpha(-435153640, alpha);
            ...
            this.drawPriceLabel(graphics, (String)buyText, cosmetic.getPrice() > 0, btnX, btnY, btnW, VayuHUDUI.withAlpha(canAfford ? -723724 : -9934744, alpha), alpha);
        }
    }
    ...
    private void drawPriceLabel(GuiGraphicsExtractor graphics, String text, boolean showDiamond, int x, int y, int w, int textColor, int alpha) {
        ...
        graphics.blit(RenderPipelines.GUI_TEXTURED, DisplaySpace.texture(DIAMOND_TEXTURE), startX, y + 5, 0.0f, 0.0f, iconSize, iconSize, iconSize, iconSize, VayuHUDUI.withAlpha(-13312, alpha));
    ...
    private void drawPlaceholderPreview(GuiGraphicsExtractor graphics, StoreCosmetic cosmetic, int x, int y, int size, int alpha) {
        int bg = VayuHUDUI.blend(-435153640, cosmetic.getRarity().getColor(), 0.22f);
        graphics.fill(x, y, x + size, y + size, VayuHUDUI.withAlpha(bg, alpha));
        VayuHUDUI.outline(graphics, x, y, size, size, VayuHUDUI.withAlpha(cosmetic.getRarity().getColor(), alpha));
        int accent = VayuHUDUI.withAlpha(cosmetic.getRarity().getColor(), Math.min(alpha, 150));
        ...
        this.drawScaledCenteredText(graphics, glyph, x + size / 2, y + size / 2, scale, VayuHUDUI.withAlpha(-723724, alpha), true);
    }
