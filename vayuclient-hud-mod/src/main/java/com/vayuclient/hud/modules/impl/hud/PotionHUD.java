...
extends Module {
    ...
    public void onRender(GuiGraphicsExtractor graphics, float tickDelta) {
        ...
        Collection<MobEffectInstance> effects = PotionHUD.mc.player.getActiveEffects();
        if (effects.isEmpty()) {
        ...
        if (((Boolean)this.background.getValue()).booleanValue()) {
            VayuHUDUI.hudPanel(graphics, x - 5, y - 4, maxWidth + 10, effects.size() * lineHeight + 6, this.bgOpacity.getIntValue());
        }
        for (MobEffectInstance effect : effects) {
            int n;
