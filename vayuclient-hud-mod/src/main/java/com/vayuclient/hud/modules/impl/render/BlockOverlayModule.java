...
extends Module {
    ...
    private final BooleanSetting hudBackground = this.register(new BooleanSetting("hud_background", "Show HUD background", false));
    private final ColorSetting hudBgColor = this.register(new ColorSetting("hud_bg_color", "HUD background color", 0, 0, 0, 180));
    private final NumberSetting hudBgOpacity = this.register(new NumberSetting("hud_bg_opacity", "HUD background opacity", 0.0, 0.0, 255.0, 1.0));
    ...
    public void onRender(GuiGraphicsExtractor graphics, float tickDelta) {
        ...
        if (((Boolean)this.hudBackground.getValue()).booleanValue()) {
            ...
            VayuHUDUI.roundedRect(graphics, x, y, hudWidth, hudHeight, 4, backgroundColor);
            VayuHUDUI.outline(graphics, x, y, hudWidth, hudHeight, 1154997472);
        }
