...
extends Module {
    ...
    public void onRender(GuiGraphicsExtractor graphics, float tickDelta) {
        ...
        for (int i = 0; i < Math.min(notifications.size(), 5); ++i) {
            ...
            int panelColor = VayuHUDUI.withAlpha(-1475275496, (int)(alpha * 220.0f));
            int borderColor = VayuHUDUI.withAlpha(1725422816, (int)(alpha * 160.0f));
            int textColor = 0xFFFFFF | (int)(alpha * 255.0f) << 24;
            int subtextColor = 0xD1D5D8 | (int)(alpha * 255.0f) << 24;
            VayuHUDUI.roundedRect(graphics, x, y, notifWidth, notifHeight, 4, panelColor);
            VayuHUDUI.outline(graphics, x, y, notifWidth, notifHeight, borderColor);
            graphics.fill(x + 1, y + 1, x + 4, y + notifHeight - 1, VayuHUDUI.withAlpha(accentColor, (int)(alpha * 255.0f)));
