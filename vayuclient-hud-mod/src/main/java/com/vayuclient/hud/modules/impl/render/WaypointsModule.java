...
extends Module {
    ...
    private final Path configPath = FabricLoader.getInstance().getConfigDir().resolve("vayuclient-hud").resolve("waypoints.json");
    ...
    private void load() {
        ...
        try {
            List<Waypoint> loaded = this.gson.fromJson(Files.readString(this.configPath), WAYPOINT_LIST);
            if (loaded != null) {
                loaded.removeIf((Waypoint waypoint) -> waypoint == null || waypoint.id == null || waypoint.name == null);
