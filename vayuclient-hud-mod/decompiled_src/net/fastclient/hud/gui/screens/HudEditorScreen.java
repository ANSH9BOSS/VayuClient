...
extends Screen {
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        graphics.fill(0, 0, this.screenWidth(), this.screenHeight(), FastClientUI.withAlpha(-16777216, 80));
    }
    ...
    private void renderCenterGuides(GuiGraphicsExtractor graphics) {
        int centerColor = FastClientUI.withAlpha(-39373, 45);
    ...
    private void renderGrid(GuiGraphicsExtractor graphics) {
        int lineColor = FastClientUI.withAlpha(1154997472, 28);
    ...
    private void renderDraggableModule(GuiGraphicsExtractor graphics, DraggableModule dm, int mouseX, int mouseY) {
        ...
        if (isDragging) {
            int bracketLen = 6;
            int bracketColor = FastClientUI.withAlpha(-39373, 220);
            ...
            int bw = dm.width;
            int bh = dm.height;
            graphics.fill(bx - 1, by - 1, bx + bracketLen, by, bracketColor);
            graphics.fill(bx - 1, by - 1, bx, by + bracketLen, bracketColor);
            graphics.fill(bx + bw - bracketLen, by - 1, bx + bw + 1, by, bracketColor);
            graphics.fill(bx + bw, by - 1, bx + bw + 1, by + bracketLen, bracketColor);
            graphics.fill(bx - 1, by + bh, bx + bracketLen, by + bh + 1, bracketColor);
            graphics.fill(bx - 1, by + bh - bracketLen, bx, by + bh + 1, bracketColor);
            graphics.fill(bx + bw - bracketLen, by + bh, bx + bw + 1, by + bh + 1, bracketColor);
            graphics.fill(bx + bw, by + bh - bracketLen, bx + bw + 1, by + bh + 1, bracketColor);
            return;
        }
        if (isResizing) {
            borderColor = FastClientUI.withAlpha(-39373, 220);
            borderWidth = 2;
        } else if (isSelected) {
            borderColor = FastClientUI.withAlpha(-39373, 210);
            borderWidth = 2;
        } else if (isHovered) {
            borderColor = FastClientUI.withAlpha(-39373, 190);
            borderWidth = 2;
        } else {
            borderColor = FastClientUI.withAlpha(1143616571, 190);
            borderWidth = 2;
        }
        boolean hasOffset = !dm.module.getName().equals("Block Overlay");
        int pad = hasOffset ? (int)(2.0f * dm.scale) : 0;
    ...
    private void renderToolbar(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        FastClientUI.roundedRect(graphics, toolbarX, toolbarY, toolbarWidth, currentHeight, 5, FastClientUI.withAlpha(-234156528, 205));
        FastClientUI.outline(graphics, toolbarX, toolbarY, toolbarWidth, currentHeight, FastClientUI.withAlpha(1143616571, 190));
        graphics.fill(toolbarX, toolbarY, toolbarX + toolbarWidth, toolbarY + 1, FastClientUI.withAlpha(-39373, 170));
        graphics.text(this.font, "HUD Editor", toolbarX + 8, toolbarY + 6, FastClientUI.withAlpha(-723724, 210), false);
        String gridInfo = this.gridSize + "px";
        graphics.text(this.font, gridInfo, toolbarX + toolbarWidth - this.font.width(gridInfo) - 30, toolbarY + 6, FastClientUI.withAlpha(-7303024, 165), false);
        String collapseIcon = this.toolbarExpanded ? "\u25b2" : "\u25bc";
        graphics.text(this.font, collapseIcon, toolbarX + toolbarWidth - 16, toolbarY + 6, FastClientUI.withAlpha(-7303024, 170), false);
        if (animVal > 0.5f) {
            int btnX;
            ...
            int gridBtnColor = this.gridEnabled ? FastClientUI.withAlpha(-39373, 165) : -435153640;
            FastClientUI.roundedRect(graphics, btnX, btnY, gridBtnWidth, btnHeight, 3, gridBtnColor);
            graphics.text(this.font, gridText, btnX + 6, btnY + 5, this.gridEnabled ? -723724 : -7303024, false);
            ...
            int snapBtnColor = this.snapToGrid ? FastClientUI.withAlpha(-39373, 165) : -435153640;
            FastClientUI.roundedRect(graphics, btnX, btnY, snapBtnWidth, btnHeight, 3, snapBtnColor);
            graphics.text(this.font, snapText, btnX + 6, btnY + 5, this.snapToGrid ? -723724 : -7303024, false);
            ...
            int guidesBtnColor = this.showGuides ? FastClientUI.withAlpha(-39373, 165) : -435153640;
            FastClientUI.roundedRect(graphics, btnX, btnY, guidesBtnWidth, btnHeight, 3, guidesBtnColor);
            graphics.text(this.font, guidesText, btnX + 6, btnY + 5, this.showGuides ? -723724 : -7303024, false);
            int infoY = btnY + btnHeight + 4;
            int infoOn = FastClientUI.withAlpha(-7303024, 150);
            int infoOff = FastClientUI.withAlpha(-9934744, 90);
    ...
    private void renderConfigPanel(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (panelAnim > 0.5f) {
            ...
            graphics.text(this.font, "Quick Actions", panelX + 10, panelY + 26, -9934744, false);
            int actionY = panelY + 45;
            graphics.text(this.font, FastClientFonts.materialSymbolLabel("\uf053", "Reset Position"), panelX + 10, actionY, -7303024, false);
            graphics.text(this.font, FastClientFonts.materialSymbolLabel("\ue00f", "Center Horizontal"), panelX + 10, actionY += 22, -7303024, false);
            graphics.text(this.font, FastClientFonts.materialSymbolLabel("\ue011", "Center Vertical"), panelX + 10, actionY += 22, -7303024, false);
            graphics.text(this.font, FastClientFonts.materialSymbolLabel("\ue8b8", "Full Settings..."), panelX + 10, actionY += 28, -39373, false);
        }
    }
    ...
    private void renderPositionInfo(GuiGraphicsExtractor graphics, DraggableModule dm) {
        ...
        FastClientUI.roundedRect(graphics, infoX - 5, infoY - 3, posWidth + 10, 14, 3, -435153640);
        graphics.text(this.font, posText, infoX, infoY, FastClientUI.withAlpha(-7303024, 210), false);
    }
    ...
    private void renderFooter(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(this.font, hints, (this.screenWidth() - hintsWidth) / 2, this.screenHeight() - 12, FastClientUI.withAlpha(-7303024, 120), false);
    }
