    ...
    private <T> CompletableFuture<T> supplyAsync(ThrowingSupplier<T> supplier) {
        ...
        this.executor.execute(() -> {
            try {
                ...
                FastClientHUDClient.LOGGER.warn("[MarketplaceClient] Request failed: {}", (Object)e.getMessage());
    ...
    private List<StoreCosmetic> parseCatalog(JsonArray catalogArray) {
        ...
        for (JsonElement element : catalogArray) {
            ...
            try {
                ...
                FastClientHUDClient.LOGGER.warn("[MarketplaceClient] Skipping invalid catalog item: {}", (Object)e.getMessage());
            }
        }
        return items;
    }
